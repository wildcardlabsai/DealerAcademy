    ...
    <div className="bg-[#0a0a0a] selection:bg-blue-500/30">
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            <p className="text-zinc-400 text-lg mb-8 font-medium leading-relaxed">
              Every course module and strategy in this academy is handwritten by <strong>{EXPERT_DATA.name}</strong>. 
            </p>
          ...
          {modules.map((m, idx) => (
            <Card key={m.id} className="hover:border-zinc-700 transition-all group flex flex-col">
              <div className="mb-6 flex justify-between items-start">
                 <div className="w-10 h-10 rounded-lg bg-blue-600/10 border border-blue-500/20 flex items-center justify-center text-blue-500 font-bold">
                    {idx + 1}
                 </div>
              ...
              <div className="pt-4 border-t border-zinc-800 flex items-center gap-2 text-xs font-bold text-zinc-400">
                 <BookOpen size={14} />
                 {Math.floor(Math.random() * 5) + 8} Lessons
              </div>
            </Card>
          ))}
        </div>
      ...
      <section id="pricing" className="py-32 px-6 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h3 className="text-4xl font-bold mb-4 uppercase tracking-tighter">Choose Your Plan</h3>
          <p className="text-zinc-400">Scale your dealership with practical, dealer-led education.</p>
        </div>
        <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {/* Free Tier */}
          <Card className="border-zinc-800 flex flex-col">
            <div className="mb-8">
              <h4 className="text-xl font-bold uppercase tracking-tight mb-2">Free Starter</h4>
              <div className="text-4xl font-black mb-4">£0</div>
              <p className="text-zinc-500 text-sm font-medium">Perfect for seeing how we work.</p>
            </div>
            <ul className="space-y-4 mb-8 flex-1">
              {['First 2 Modules Fully Free', 'Introductory Templates', 'Access to Resources Vault (Limited)', 'No Credit Card Needed'].map((item, i) => (
                <li key={i} className="flex items-center gap-3 text-sm text-zinc-300">
                  <CheckCircle size={16} className="text-blue-500 shrink-0" /> {item}
                </li>
              ))}
            </ul>
            <Link to="/signup">
          ...
          <Card className="border-zinc-800 flex flex-col relative bg-zinc-900/80">
            <div className="mb-8">
              <h4 className="text-xl font-bold uppercase tracking-tight mb-2">Academy Pro</h4>
              <div className="text-4xl font-black mb-4">£12 <span className="text-lg text-zinc-500 font-medium">/mo</span></div>
              <p className="text-zinc-500 text-sm font-medium">Full access to everything we offer.</p>
            </div>
            <ul className="space-y-4 mb-8 flex-1">
              {['All 10 Core Modules', 'Full Templates Vault', 'AI Library & Prompts', 'Unlimited Resources Downloads', 'Monthly Strategy Updates'].map((item, i) => (
                <li key={i} className="flex items-center gap-3 text-sm text-zinc-300">
                  <CheckCircle size={16} className="text-blue-500 shrink-0" /> {item}
                </li>
              ))}
          ...
          <Card className="border-blue-600 flex flex-col relative shadow-xl shadow-blue-900/10 bg-blue-600/5">
            <div className="absolute -top-3 left-1/2 -translate-x-1/2">
              <Badge variant="success">Best Value — Save 30%</Badge>
            </div>
            <div className="mb-8">
              <h4 className="text-xl font-bold uppercase tracking-tight mb-2">Academy Annual</h4>
              <div className="text-4xl font-black mb-4">£99 <span className="text-lg text-zinc-500 font-medium">/year</span></div>
              <p className="text-zinc-500 text-sm font-medium">The most committed dealers win.</p>
            </div>
            <ul className="space-y-4 mb-8 flex-1">
              {['Everything in Monthly Plan', '30% Discount over Monthly', 'Lock in Launch Pricing Forever', 'Priority Support Access'].map((item, i) => (
                <li key={i} className="flex items-center gap-3 text-sm text-zinc-300">
                  <CheckCircle size={16} className="text-blue-500 shrink-0" /> {item}
                </li>
              ))}
            ...
            <Link to="/signup">
              <Button className="w-full h-12">Choose Yearly</Button>
            </Link>
          </Card>
        </div>
      </section>
      ...
      <footer className="border-t border-zinc-900 py-24 px-6 bg-zinc-950">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between gap-16">
          <div className="max-w-sm">
            ...
            <p className="text-zinc-500 leading-relaxed text-sm">The leading education platform for UK independent car dealers who want to scale through smarter marketing.</p>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-16">
            <div>
              <h5 className="font-bold mb-6 text-sm uppercase tracking-widest text-zinc-600">Learning</h5>
              <ul className="space-y-4 text-zinc-400 text-sm font-medium">
                <li><button onClick={() => scrollTo('curriculum')} className="hover:text-blue-500">Curriculum</button></li>
                <li><Link to="/signup" className="hover:text-blue-500">Pre-Registration</Link></li>
              </ul>
            </div>
            <div>
              <h5 className="font-bold mb-6 text-sm uppercase tracking-widest text-zinc-600">Company</h5>
              <ul className="space-y-4 text-zinc-400 text-sm font-medium">
        ...
        <div className="max-w-7xl mx-auto mt-24 pt-8 border-t border-zinc-900 text-zinc-600 text-xs flex flex-col sm:flex-row justify-between items-center gap-4">
          <p>© 2024 Dealer Growth Academy. Built for Independent Car Dealers.</p>
    ...
    <div className="max-w-6xl mx-auto">
      <header className="mb-12">
        <div className="flex justify-between items-start">
           <div>
              ...
              <p className="text-zinc-500 font-medium italic">Courses launch on March 1st 2026. Your first two modules are ready for preview.</p>
           </div>
           <Link to="/courses">
             <Button variant="outline" className="hidden sm:flex">View Academy</Button>
           </Link>
        </div>
      </header>
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-12">
        <Card className="lg:col-span-2 bg-gradient-to-br from-zinc-900/50 to-zinc-950 p-8 border-blue-500/10">
          <div className="flex flex-col lg:flex-row justify-between items-center gap-8 mb-6">
            <div className="text-center lg:text-left">
              ...
              <h3 className="text-3xl font-black mt-2 mb-4 uppercase tracking-tight">Official Release in:</h3>
              <CountdownTimer />
            </div>
            <div className="bg-zinc-950 p-6 rounded-2xl border border-zinc-800 text-center lg:text-right w-full lg:w-auto">
              <p className="text-xs text-zinc-500 font-black uppercase tracking-[0.2em] mb-2">Member Tier</p>
              <h4 className="text-2xl font-black text-blue-500 uppercase tracking-tighter">{user?.tier}</h4>
              <p className="text-[10px] text-zinc-600 mt-1 uppercase font-bold tracking-widest">Active Member</p>
            </div>
          </div>
          <div className="pt-8 border-t border-zinc-900 mt-6 text-center lg:text-left">
            <div className="flex items-center gap-3 text-sm text-zinc-400 bg-zinc-900/50 p-4 rounded-xl border border-zinc-800 justify-center lg:justify-start">
               <AlertCircle size={20} className="text-blue-500 shrink-0" />
               <p>Registered users can view the curriculum today, but lessons will unlock on <span className="text-white font-bold">1st March 2026</span>.</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="flex flex-col bg-zinc-900/30 border-emerald-500/10">
          <div className="mb-auto">
            <Badge variant="success">Launch Promo</Badge>
            <h3 className="text-xl font-bold mt-3 mb-2 uppercase tracking-tighter leading-tight">First 25 Promotion</h3>
            <p className="text-sm text-zinc-500 font-medium leading-relaxed mb-6">
               The first 25 dealers who registered will receive a personal email with their <span className="text-white font-bold underline font-semibold italic">Lifetime Free Access</span> code on launch day.
            </p>
            <div className="p-3 bg-emerald-500/5 border border-emerald-500/20 rounded-lg text-xs font-bold text-emerald-500 text-center uppercase tracking-widest">
               You are registered
            </div>
          </div>
        </Card>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
           <section>
              <h4 className="text-xs uppercase tracking-widest font-black text-zinc-500 mb-6 flex items-center gap-2">
                 <ArrowRight size={14} className="text-blue-500" /> Academy Roadmap
              </h4>
              <div className="grid sm:grid-cols-2 gap-4">
                 {modules.slice(0, 4).map((m, i) => (
                    <Link key={m.id} to={`/courses`}>
                       <Card className="hover:border-zinc-700 transition-colors p-5 h-full group">
                          <div className="text-blue-500 font-black text-xs mb-3">MODULE 0{i+1}</div>
                          <h5 className="font-bold mb-2 group-hover:text-blue-400 transition-colors uppercase tracking-tight">{m.title}</h5>
                          <p className="text-xs text-zinc-500 font-medium line-clamp-2">{m.description}</p>
                          {i < 2 && <Badge variant="success">Free Access</Badge>}
                       </Card>
                    </Link>
                 ))}
              </div>
        ...
        <div>
           <h4 className="text-xs uppercase tracking-widest font-black text-zinc-500 mb-6 uppercase tracking-widest">Coming Soon</h4>
           <div className="space-y-3">
              {[
                { l: 'Templates Vault', i: FileText, p: '/templates', c: 'text-blue-500' },
                { l: 'Resources', i: Download, p: '/resources', c: 'text-emerald-500' },
                { l: 'Account Settings', i: Settings, p: '/settings', c: 'text-zinc-500' }
              ].map((item, i) => (
                <Link key={i} to={item.p}>
                  <Card className="p-4 flex items-center justify-between hover:bg-zinc-800/50 transition-all group">
                     <div className="flex items-center gap-3">
...
const CourseListing = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <header className="mb-12">
        <Badge variant="warning">Release Date: 1st March 2026</Badge>
        <h2 className="text-4xl font-black mt-4 mb-4 uppercase tracking-tighter">Full Curriculum</h2>
        <p className="text-zinc-500 font-medium max-w-2xl leading-relaxed">
           The first two modules are free for all members. Modules 3-10 require an Academy Pro subscription.
        </p>
      </header>

      <div className="space-y-8">
        {modules.map((module, mIdx) => {
          const isModuleFree = mIdx < 2;
          const isLocked = !isModuleFree && user?.tier === 'FREE';

          return (
            <div key={module.id} className={`relative group ${isLocked ? 'grayscale opacity-60' : ''}`}>
              <div className={`flex flex-col lg:flex-row gap-10 p-8 rounded-2xl border transition-all bg-zinc-900/30 border-zinc-800 ${isLocked ? 'bg-zinc-950/20' : ''}`}>
                <div className="lg:w-1/3">
                  <div className="flex items-center gap-4 mb-4">
                    <span className={`text-3xl font-black ${isLocked ? 'text-zinc-800' : 'text-blue-500'}`}>0{mIdx + 1}</span>
                    <div>
                      <h3 className="text-xl font-bold uppercase tracking-tight">{module.title}</h3>
                      <div className="flex gap-2 mt-1">
                        <Badge variant="neutral">{module.lessons.length} LESSONS</Badge>
                        {isModuleFree && <Badge variant="success">FREE ACCESS</Badge>}
                        {isLocked && <Badge variant="danger">PRO ONLY</Badge>}
                      </div>
                    </div>
                  </div>
                  <p className="text-sm text-zinc-500 font-medium leading-relaxed mb-6">{module.description}</p>
                </div>

                <div className="lg:w-2/3 grid sm:grid-cols-2 gap-3 relative">
                  {module.lessons.map((lesson) => {
                    return (
                      <div key={lesson.id} className="cursor-not-allowed group/lesson">
                        <div className="p-4 bg-zinc-950/80 border border-zinc-900 rounded-xl flex items-center justify-between text-zinc-600 transition-all">
                          <div className="flex items-center gap-3">
                            {isLocked ? <Lock size={14} className="text-zinc-700" /> : <Clock size={14} className="text-zinc-700" />}
                  ...
                  {isLocked && (
                    <div className="absolute inset-0 bg-black/40 backdrop-blur-[2px] rounded-xl flex items-center justify-center">
                       <Link to="/settings" className="z-10">
                         <Button variant="primary" className="shadow-2xl">Upgrade to Unlock</Button>
                       </Link>
                    </div>
                  )}
                </div>
...
const TemplatesVault = () => {
  ...
  const filtered = INITIAL_TEMPLATES.filter(t => 
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <header className="mb-12">
        <Badge variant="info">Coming 1st March 2026</Badge>
        <h2 className="text-4xl font-black mt-4 mb-4 uppercase tracking-tighter">Templates Vault</h2>
        <p className="text-zinc-500 font-medium max-w-2xl italic">The full vault will be copy-ready on launch day. Pro members get unlimited access to all assets.</p>
      </header>
      ...
      <div className="flex flex-col md:flex-row gap-6 mb-12 items-center justify-between">
          ...
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${
                filter === cat ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20' : 'bg-zinc-900 text-zinc-500 hover:text-white border border-zinc-800'
              }`}
            >
              {cat}
            </button>
        ...
        <div className="relative w-full md:w-80">
           ...
           <input 
             ...
             className="w-full bg-zinc-950 border border-zinc-800 rounded-xl pl-12 pr-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all"
             value={search}
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 opacity-60">
        {filtered.map(template => (
          <Card key={template.id} className="flex flex-col relative overflow-hidden group border-zinc-900">
            <div className="flex justify-between items-start mb-6">
              <Badge variant="neutral">{template.category}</Badge>
              {template.isPaidOnly && <Badge variant="danger">PRO</Badge>}
            </div>
            <h3 className="text-xl font-bold mb-6 leading-tight uppercase tracking-tight">{template.title}</h3>
            <div className="bg-zinc-950 p-6 rounded-xl border border-zinc-900 flex-1 mb-8 font-mono text-[13px] text-zinc-700 whitespace-pre-wrap leading-relaxed blur-[2px] select-none">
              {template.content}
            </div>
            <Button variant="ghost" className="w-full py-3 cursor-not-allowed border border-zinc-900" disabled>
               Unlock on 1st March
            </Button>
          </Card>
        ))}
...
const ResourcesPage = () => {
   const resources = [
      { t: 'Marketing Audit Checklist', d: 'Step-by-step checklist to audit your digital presence.', f: 'PDF' },
      { t: 'Video Walkaround Storyboard', d: 'Plan your shots before you start filming.', f: 'DOCX' },
      { t: 'Lead Tracking Spreadsheet', d: 'Simple tool to track source and conversion of every lead.', f: 'XLSX' },
      { t: 'Weekly Social Calendar', d: 'A visual guide to what to post and when.', f: 'PDF' },
   ];

   return (
    <div className="max-w-6xl mx-auto">
      <header className="mb-12">
        <Badge variant="info">Toolkit Preview</Badge>
        <h2 className="text-4xl font-black mt-4 mb-4 uppercase tracking-tighter">Resources Vault</h2>
        <p className="text-zinc-500 font-medium max-w-2xl leading-relaxed italic">Downloadable assets will be active on 1st March 2026.</p>
      </header>

      <div className="grid md:grid-cols-2 gap-6 opacity-50">
         {resources.map((res, i) => (
            <Card key={i} className="flex flex-col border-zinc-900 grayscale">
               <div className="flex justify-between items-start mb-6">
                  <div className="p-3 bg-zinc-900 rounded-xl text-zinc-600 border border-zinc-800">
                     <Download size={24} />
                  </div>
                  <Badge variant="neutral">{res.f}</Badge>
               </div>
               <h3 className="text-xl font-bold mb-3 uppercase tracking-tight">{res.t}</h3>
               <p className="text-zinc-500 text-sm font-medium leading-relaxed mb-8">{res.d}</p>
               <Button variant="outline" className="w-full cursor-not-allowed" disabled>Awaiting Launch</Button>
            </Card>
         ))}
      </div>
    </div>
...
const SettingsPage = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <header className="mb-12">
        <h2 className="text-4xl font-black mb-4 text-zinc-100 uppercase tracking-tighter">Member Settings</h2>
        <p className="text-zinc-500 font-medium">Your account details and pre-registration status.</p>
      </header>
      ...
      <div className="space-y-8">
        <Card className="bg-gradient-to-br from-zinc-900 to-zinc-950 p-8 border-zinc-800">
          <div className="flex justify-between items-start mb-8">
             <div>
                <h3 className="text-2xl font-black mb-2 tracking-tight">Your Status</h3>
                <p className="text-zinc-500 text-sm font-medium">Academy Member Since {new Date(user?.createdAt || '').toLocaleDateString()}</p>
             </div>
             <Badge variant={user?.tier === 'PAID' ? 'success' : 'neutral'}>{user?.tier === 'PAID' ? 'PRO MEMBER' : 'FREE MEMBER'}</Badge>
          </div>

          <div className="flex items-center gap-6 p-6 bg-zinc-950 border border-zinc-900 rounded-2xl mb-8">
             <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shadow-xl ${user?.tier === 'PAID' ? 'bg-blue-500/10 text-blue-500 border-blue-500/20' : 'bg-zinc-800 text-zinc-600 border-zinc-700'}`}>
                {user?.tier === 'PAID' ? <Zap size={32} /> : <Lock size={32} />}
             </div>
             <div className="flex-1">
                <p className="font-black text-xl mb-1 uppercase tracking-tighter">{user?.tier} PLAN</p>
                <p className="text-sm text-zinc-500 font-medium leading-relaxed italic">
                   {user?.tier === 'FREE' 
                     ? 'Upgrade to Academy Pro to unlock Modules 3-10, the full Templates Vault, and our AI library.'
                     : 'You have full access to the Academy. Your first billing cycle starts March 1st 2026.'}
                </p>
             </div>
             {user?.tier === 'FREE' && (
               <Button onClick={() => updateTier('PAID')} variant="primary" className="shrink-0">Upgrade Now</Button>
             )}
          </div>
        </Card>

        <Card>
          <h3 className="text-xl font-bold mb-8 uppercase tracking-tight">Dealership Profile</h3>
          <div className="grid sm:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-zinc-600">Dealership Name</label>
              <input readOnly value={user?.dealershipName} className="w-full bg-zinc-950 border border-zinc-900 rounded-xl px-4 py-3 text-zinc-400 outline-none" />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-zinc-600">Primary Contact</label>
              <input readOnly value={user?.name} className="w-full bg-zinc-950 border border-zinc-900 rounded-xl px-4 py-3 text-zinc-400 outline-none" />
            </div>
          </div>
        </Card>
      </div>
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <header className="mb-12 flex justify-between items-center">
        <div>
          <Badge variant="warning">ADMINSTRATION</Badge>
          <h2 className="text-4xl font-black mt-4 mb-2 uppercase tracking-tighter">Course Management</h2>
          <p className="text-zinc-500 font-medium">Admin panel remains active for curriculum prep.</p>
        </div>
        <Button><Plus size={18} /> New Module</Button>
      </header>

      <div className="space-y-6">
        {modules.map((m, idx) => (
          <Card key={m.id} className="bg-zinc-900/40 border-zinc-800">
             <div className="flex justify-between items-center mb-8">
                <div className="flex items-center gap-4">
                   <div className="w-10 h-10 rounded-lg bg-zinc-950 border border-zinc-800 flex items-center justify-center font-black text-zinc-500">{idx+1}</div>
                   <div>
                      <h3 className="text-xl font-bold uppercase tracking-tight">{m.title}</h3>
                      <p className="text-xs text-zinc-500 font-bold">{m.lessons.length} LESSONS</p>
                   </div>
                </div>
...
const AuthPage = ({ mode }: { mode: 'login' | 'signup' }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Authentication failed. Check your details.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          <div className="text-center mb-12">
            <Link to="/" className="text-2xl font-black bg-gradient-to-r from-blue-400 to-emerald-400 bg-clip-text text-transparent uppercase tracking-tight">
              Dealer Growth Academy
            </Link>
            <h2 className="text-2xl font-black text-white mt-12 tracking-tight uppercase tracking-tight">
              {mode === 'login' ? 'Member Login' : 'Secure Your Place'}
            </h2>
            <p className="text-zinc-500 font-medium text-sm mt-2 italic">
              {mode === 'login' ? 'Log in to manage your pre-registration.' : 'The first 2 modules are fully free for all pre-registered users.'}
            </p>
          </div>

          <Card className="bg-zinc-900/30 p-8 border-zinc-800 shadow-2xl">
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    <input 
                      required 
                      type="text" 
                      placeholder="e.g. Prestige Motors UK"
                      value={dealership} 
              ...
              <Button type="submit" className="w-full py-4 text-lg font-black uppercase tracking-widest" disabled={isLoading}>
                {isLoading ? 'Authorising...' : mode === 'login' ? 'Login Now' : 'Join the Academy'}
              </Button>
            </form>
          </Card>
          ...
          <div className="text-center mt-10">
            <p className="text-sm text-zinc-600 font-bold uppercase tracking-widest">
              {mode === 'login' ? (
                <>New here? <Link to="/signup" className="text-blue-500 hover:text-white transition-colors">Pre-register free</Link></>
              ) : (
                <>Already a member? <Link to="/login" className="text-blue-500 hover:text-white transition-colors">Sign in here</Link></>
              )}
            </p>
          </div>
        </div>
...
const App = () => {
  ...
  const [modules, setModules] = useState<Module[]>(() => {
    const saved = localStorage.getItem('dga_modules');
    return saved ? JSON.parse(saved) : INITIAL_MODULES;
  });

  const [progress, setProgress] = useState<UserProgress>(() => {
    const saved = localStorage.getItem('dga_progress');
    return saved ? JSON.parse(saved) : { completedLessonIds: [] };
  });
  ...
  useEffect(() => {
    localStorage.setItem('dga_modules', JSON.stringify(modules));
  }, [modules]);

  useEffect(() => {
    localStorage.setItem('dga_progress', JSON.stringify(progress));
  }, [progress]);
  ...
  const login = async (email: string, pass: string) => {
    const mockUser: User = {
      ...
      tier: email.includes('admin') ? 'ADMIN' : 'FREE',
  ...
  const signup = async (data: Partial<User>) => {
    const newUser: User = {
      ...
      email: data.email || '',
      name: data.name || 'User',
      dealershipName: data.dealershipName || 'Independent Dealer',
      country: 'United Kingdom',
      tier: 'FREE',
      createdAt: new Date().toISOString()
    };
      ...
      <AuthContext.Provider value={{ user, login, signup, logout, updateTier }}>
        <CourseContext.Provider value={{ modules, progress, toggleLessonComplete, updateModules: setModules }}>
          <Routes>
            ...
            <Route path="/course/:moduleId/:lessonId" element={user ? <AuthenticatedLayout><CourseListing /></AuthenticatedLayout> : <Navigate to="/login" />} />
